...
从当前版本开始，项目自带一个**内置 Web 控制台**，不依赖额外前端开发服务器，适合 Docker / 直接运行 Python / exe 三种方式统一使用。默认配置如下（可在 `config/config.ini` 中修改）：

- `是否启用Web控制台(是/否) = 是`
- `Web控制台监听地址 = auto`
  - `auto` 在 Docker / 容器里会自动绑定 `0.0.0.0`
...
- `URL_config.ini` 保存后，主循环通常会在约 **3 秒** 内重新读取
- 注释掉某个正在录制的地址（行首加 `#`）后，主循环通常会先在约 **3 秒** 内识别到该变更，随后当前录制任务会在下一秒内收到停止信号并先保存已有内容
- 大多数录制/推送/代理参数会在后续轮询或下一次新录制任务中生效
- `language(zh_cn/en)`、代理检测开关、Web 控制台监听地址 / 端口 / 开关属于**启动级配置**，建议重启程序后生效

#### 访问方式

- **CLI / Python 直接运行**：启动 `python main.py` 后，默认访问 `http://127.0.0.1:18080`
- **Docker**：映射 `18080:18080` 后，访问 `http://宿主机IP:18080`
...
> README 里仅保留本 Fork 前端更新 + 上游最近 3 次主要更新；更早历史请查看 `CHANGELOG.md`。

- 20260501（本 Fork）
  - 新增内置 Web 控制台，统一支持 Docker / Python / exe 三种使用方式
  - 新增抖音值守面板、最近录制详情、录制文件筛选、磁盘告警与异常提示
  - 新增录制任务控制（暂停 / 继续 / 停止 / 重新录制）
  - 新增 `config.ini` / `URL_config.ini` 在线编辑与配置热重载
  - 发布 Windows `exe` 与 `win64.zip` 完整包，补充 ghcr.io 镜像说明
...
- 20250127（上游）
  - 新增淘宝、京东、Faceit 直播录制
  - 修复小红书直播流录制以及转码问题
  - 修复畅聊、VV 星球、FlexTV 直播录制
  - 修复批量微信直播推送
  - 新增 email SSL / port 配置、强制转 H.264 配置，并更新 ffmpeg 版本
  - 重构包为异步函数
